...
exports.addNextStepIdToPreviousStep = functions.database.ref('/users/{uid}/hunts/saved/{createdHunt}/steps/{currentStep}/order')
    .onWrite((change, context) => __awaiter(this, void 0, void 0, function* () {
    if (!change.after.exists()) {
        return null;
    }
    ...
    try {
        let previousStepId;
        const stepsSnapshot = yield admin.database().ref('users').child(uid).child('hunts/saved').child(createdHunt).child('steps').once('value');
        stepsSnapshot.forEach((stepSnapshot) => {
            if (stepSnapshot.key == currentStepId)
                return false;
